...
TEST_ADMIN_PASSWORD = os.environ.get("EASY_TEST_ADMIN_PASSWORD", "rkrhtlak1")
...
def login(req: LoginRequest):
    ...
    if not test_admin_pw_hash:
        test_admin_pw_hash = hash_pw(TEST_ADMIN_PASSWORD)
        c.execute("UPDATE room SET test_admin_pw=%s WHERE room_id=%s", (test_admin_pw_hash, req.room_id))
        conn.commit()
    c.close()
    conn.close()
    ...
    if verify_pw(req.password, admin_pw_hash):
        role = "admin"
        nickname = "방장"
    elif verify_pw(req.password, test_admin_pw_hash):
        role = "admin"
        nickname = "방장(테스트)"
    ...
    try:
        ...
        c.execute(
            ...
            (rid, room.title, hash_pw(room.admin_pw), hash_pw(room.team_pw) if room.team_pw else "",
             room.city or "", currency, room.member_count or 1, room.is_comment_enabled, hash_pw(TEST_ADMIN_PASSWORD))
        )
        conn.commit()
        c.close()
        conn.close()
