...
# 앱 초기화
# ─────────────────────────────────────────────
app = FastAPI(title="Easy Schedule Pro API", version="4.0.1")
...
def create_room(room: RoomCreate):
    try:
        ...
        conn = get_db_connection(); c = conn.cursor()
        c.execute("""INSERT INTO room (room_id, title, admin_pw, team_pw, city, currency, member_count, is_comment_enabled)
                     VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
                  (rid, room.title, hash_pw(room.admin_pw), hash_pw(room.team_pw) if room.team_pw else "",
                   room.city or "", curr, room.member_count or 1, room.is_comment_enabled))
...
def get_room_data(room_id: str, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    conn = get_db_connection(); c = conn.cursor()
    c.execute("""SELECT title, city, currency, member_count, is_comment_enabled,
    ...
    (title, city, currency, member_count, is_comment_enabled, b_n1, b_l1, b_n2, b_l2, b_n3, b_l3) = room_row
    ...
    c.execute("""SELECT id, day_num, start_time, end_time, content, author, google_map_url, tabelog_url, budget, place_id, rating
                 FROM schedule WHERE room_id=%s ORDER BY day_num ASC, sort_order ASC""", (room_id,))
    ...
    c.close(); conn.close()
    return {"room_id": room_id, "title": title, "city": city, "currency": currency, "currency_symbol": CURRENCY_SYMBOL_MAP.get(currency, ""), "member_count": member_count, "is_comment_enabled": is_comment_enabled, "role": role, "nickname": nickname, 
            "bookmarks": [{"name": b_n1, "url": b_l1}, {"name": b_n2, "url": b_l2}, {"name": b_n3, "url": b_l3}], 
...
def reorder_schedule(room_id: str, req: ReorderRequest, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    if role not in ("admin", "team"): raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    for idx, sid in enumerate(req.new_order):
        c.execute("UPDATE schedule SET sort_order=%s WHERE id=%s AND room_id=%s", (idx, sid, room_id))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def add_flight(room_id: str, fl: FlightCreate, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    if role not in ("admin", "team"): raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("INSERT INTO flight (room_id, flight_type, airport, flight_num, terminal, departure_time, arrival_time, memo) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)", (room_id, fl.flight_type, fl.airport, fl.flight_num, fl.terminal, fl.departure_time, fl.arrival_time, fl.memo))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def delete_flight(room_id: str, fid: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    if role != "admin": raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("DELETE FROM flight WHERE id=%s AND room_id=%s", (fid, room_id))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def add_acc(room_id: str, acc: AccommodationCreate, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    if role not in ("admin", "team"): raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("INSERT INTO accommodation (room_id, days_applied, hotel_name, google_map_url, has_breakfast, budget) VALUES (%s, %s, %s, %s, %s, %s)", (room_id, ",".join(map(str, sorted(acc.days_applied))), acc.hotel_name, acc.google_map_url, acc.has_breakfast, acc.budget))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def delete_acc(room_id: str, aid: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    if role != "admin": raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("DELETE FROM accommodation WHERE id=%s AND room_id=%s", (aid, room_id))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def vote_suggestion(room_id: str, sid: int, type: str):
    col = "good_cnt" if type == "good" else "bad_cnt"
    conn = get_db_connection(); c = conn.cursor()
    c.execute(f"UPDATE suggestion SET {col} = {col} + 1 WHERE id=%s AND room_id=%s", (sid, room_id))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}

@app.post("/room/{room_id}/suggestion/{sid}/approve")
def approve_suggestion(room_id: str, sid: int, req: ApproveRequest, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    role, nickname = get_current_user_info(room_id, credentials)
    if role != "admin": raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("SELECT content, google_map_url, tabelog_url FROM suggestion WHERE id=%s AND room_id=%s", (sid, room_id))
    row = c.fetchone()
    if row:
...
def delete_suggestion(room_id: str, sid: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    role, _ = get_current_user_info(room_id, credentials)
    if role != "admin": raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("DELETE FROM suggestion WHERE id=%s AND room_id=%s", (sid, room_id))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
...
def add_comment(room_id: str, sid: int, cm: CommentCreate, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    ...
    row = c.fetchone()
    if not row or not row[0]: c.close(); conn.close(); raise HTTPException(status_code=403)
    writer = nickname if role in ("admin", "team") else f"훈수꾼 {cm.writer_name[:4]}"
    c.execute("INSERT INTO comment (schedule_id, writer_name, content) VALUES (%s, %s, %s)", (sid, writer, cm.content))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}

@app.delete("/room/{room_id}/schedule/{sid}/comment/{cid}")
def delete_comment(room_id: str, sid: int, cid: int, credentials: Optional[HTTPAuthorizationCredentials] = Depends(bearer_scheme)):
    role, _ = get_current_user_info(room_id, credentials)
    if role != "admin": raise HTTPException(status_code=403)
    conn = get_db_connection(); c = conn.cursor()
    c.execute("DELETE FROM comment WHERE id=%s AND schedule_id=%s", (cid, sid))
    conn.commit(); c.close(); conn.close(); return {"status": "ok"}
